...
| Single read path enforcement (R5 F2)    | Handler reads of window/session state go through `window_query` / `session_query`; direct `session_manager.<attr>` in `handlers/**` restricted to write/admin allow-list. AST walk over 81 handler files asserts the rule (`test_query_layer_only_for_handlers.py`)                                        |
| Recovery split (R5 F3)                  | `recovery_callbacks.py` shrunk to ~80-LOC dispatcher; `recovery_banner.py` (~450 LOC dead-window UX) + `resume_picker.py` (~400 LOC resume UX + transcript scan) are siblings. `recovery/__init__.py` re-exports unchanged; pinned by `test_recovery_subpackage_surface.py`                                |
...
| Lazy-import lint enforcement (R5 F5)    | `scripts/lint_lazy_imports.py` AST-walks `src/ccgram/**/*.py` and fails any in-function import without `# Lazy:` (or inside `if TYPE_CHECKING:` / `_reset_*_for_testing`). All 175 sites annotated. Cycle test expanded from 29 → 162 modules                                                              |
