...
- [ ] Add `pathspec>=0.12.0` to `[project] dependencies`
- [ ] Run `uv sync` to install
- [ ] Verify import works: `python -c "import pathspec"`
...
- [ ] Add `send_search_depth` and `send_max_results` to config with env var loading (`CCGRAM_SEND_SEARCH_DEPTH` default 5, `CCGRAM_SEND_MAX_RESULTS` default 50)
- [ ] Create `send_security.py` with constants: `_SECRET_PATTERNS` (fnmatch list), `_EXCLUDED_DIRS` (frozenset), `_TELEGRAM_FILE_LIMIT` (50 MB)
- [ ] Implement `is_path_contained(path: Path, root: Path) -> bool` — resolve both, use `is_relative_to`
- [ ] Implement `is_hidden(path: Path, root: Path) -> bool` — check any component relative to root starts with `.`
- [ ] Implement `matches_secret_pattern(path: Path) -> str | None` — fnmatch case-insensitive, return matching pattern or None
- [ ] Implement `is_gitignored(path: Path, cwd: Path) -> bool` — try `git check-ignore -q` subprocess first; on failure (not a git repo, git not found), fall back to `pathspec`: walk parent dirs collecting `.gitignore` files, build `PathSpec`, match. Return False if both fail
- [ ] Implement `check_gitleaks_rules(path: Path, cwd: Path) -> str | None` — if `.gitleaks.toml` exists, `tomllib.load()`, iterate `rules`, match `path` regexes. Return rule id/description or None
- [ ] Implement `validate_sendable(path: Path, cwd: Path) -> str | None` — full pipeline returning specific error message or None
- [ ] Implement `is_excluded_dir(name: str) -> bool` — check `_EXCLUDED_DIRS` + starts with `.`
- [ ] Write tests for `is_path_contained` (inside, outside, `..` traversal, symlink escape, boundary at root)
- [ ] Write tests for `is_hidden` (dotfile, nested dotdir, non-hidden, root itself)
- [ ] Write tests for `matches_secret_pattern` (`.env`, `*.pem`, `*.key`, `credential.json`, normal file)
- [ ] Write tests for `is_gitignored` — mock subprocess for git path (ignored, not ignored, git error triggering pathspec fallback, non-git with .gitignore file)
- [ ] Write tests for `check_gitleaks_rules` (toml with rules, no toml, matching rule, non-matching, malformed toml)
- [ ] Write tests for `validate_sendable` (each denial reason, clean file passes, correct error strings)
- [ ] Write tests for `is_excluded_dir` (node_modules, **pycache**, .git, .venv, normal dir)
...
- [ ] Add `/send` command to README.md (usage, modes, configuration)
- [ ] Add `/send` to CLAUDE.md command reference
- [ ] Add `CCGRAM_SEND_SEARCH_DEPTH` and `CCGRAM_SEND_MAX_RESULTS` to CLAUDE.md configuration table
- [ ] Update toolbar description in CLAUDE.md (provider-specific, new buttons)
- [ ] Add `pathspec` to dependency notes if applicable
